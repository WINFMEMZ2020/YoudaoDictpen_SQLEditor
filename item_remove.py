...
#change if another stript
retry_times = retry_config.get("item_remove")
...
def adb_login():
    ...
    try:
        ...
        if os.path.exists("restart.json"):
            ...
            # 保存更新后的数据回JSON文件
            with open("restart.json", "w") as file:
                json.dump(data, file)
        time.sleep(1)
        return 6
...
if retry_times >= 3:
    popout_layout = [
    [sg.Text("那个,打扰一下,但这很重要...\n在多次尝试登录adb后仍然无法登录,这很可能是adb密码不正确造成的\u3002\n请关闭此窗口,返回launcher仔细检查adb的密码,然后再试一次\u3002\n")]
]
    popout_window = sg.Window("嗯...出了点问题", layout=popout_layout)
    while True:
...
if delete_video_from_dictpen_state == 0:
    ...
    extra_text = '请注意：您目前开启了"同时删除词典笔上的视频文件"选项，删除项目后\n保存的同时会删除词典笔上的对应视频'


layout = [[sg.Text('以下是词典笔里的所有视频\n请选中需要删除的项目然后点击“删除”按钮\u3000\u3000\u3000\u3000\n\n' + extra_text,font=('SimHei', 14)),
sg.Button('移除'),
sg.Button('保存')],
[sg.Table(values=data, headings=['code', 'name'], auto_size_columns=True, max_col_width=80,
            display_row_numbers=False, justification='left', select_mode='extended', enable_events=True, num_rows=30, key='-TABLE-')],
[sg.Text("提示:可通过按住Ctrl进行多选或按住Shift进行范围选择。", font=('SimHei', 12))]
]
...
window = sg.Window('删除词典笔上的项目', layout, size=(800, 660))
#print("result=",result)
...
while True:
    ...
    if event == sg.WIN_CLOSED:
      break
        ...
        try:
            all_selected = values['-TABLE-']
            selected_items = []
            for selected_item_to_delete in all_selected[::-1]:  # 从后往前遍历以避免索引错位
                selected_row = selected_item_to_delete
